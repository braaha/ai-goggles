...
def make_mp4(
    ...
    #ffmpeg here instead of in the command window
    cmd = [
        "ffmpeg",
        "-y",
        "-i",
...
def upload_to_s3(file_path: Path, bucket: str) -> None:
    ...
    # Put files under a recordings/ 
    key = f"recordings/{file_path.name}"
    s3_uri = f"s3://{bucket}/{key}"

    cmd = ["aws", "s3", "cp", str(file_path), s3_uri]
    print("Uploading to S3:", " ".join(cmd))
    result = subprocess.run(cmd, capture_output=True, text=True)
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Record Pi camera video + mic audio, make MP4, optional S3 upload."
    )
...
def main() -> None:
    ...
    # Optional S3 upload
    if args.bucket:
        upload_to_s3(final_path, args.bucket)
